	...
	public class CustomizeAndroidWindow : Window
	{
		...
		public void PickTraitMenu(Trait oldTrait)
		{
			...
			{    
				...
				//Filter out Body Mastery trait from being choosen as it makes the venometric cell upgrade useless
				if (def.defName == "BodyMaster")
				{
					allTraits.RemoveAll(thisTrait => def.defName == thisTrait.def.defName);
				}
			}
